(function( $ ) {
	...
	$(document).on('click', '#pagination li', function(){
		$('#item-list .list tbody').empty().append('<tr><td align="text-center" colspan="6"><i class="fa fa-circle-o-notch fa-spin fa-fw"></i> 데이터를 로딩 중입니다....</td></tr>');
		let c_no = ($(this).parent().data('cno') === "undefined") ? null : $(this).parent().data('cno');
		let ref = ($(this).parent().data('ref') === "undefined") ? null : $(this).parent().data('ref');
		let ref_n = ($(this).parent().data('refn') === "undefined") ? null : $(this).parent().data('refn');
		let page = ($(this).data('page') === "undefined") ? null : $(this).data('page');
	...
	$('.new-item').click(function(){
		...
		$('#edit input, #edit textarea, #edit select').val('');
	...
	$('#edit-form').submit(function(e){
		...
		if ( $(this).find('select[name="cat-lev-1"]').val() === null ) {
			...
			$.post(ajaxurl, d).then(function(res){
				var r = $.parseJSON(res);
				if ( r.stat === 'success' ) {
					var msg = (d.data.mode === 'new') ? "아이템이 추가되었습니다." : "수정되었습니다.";
					if ( confirm(msg) ) {
						$('#search-keyword').val(r.data);
						$('#search-submit').trigger('click');
						$('#edit-form input, #edit-form textarea, #edit-form select').val('');
	...
	$('#item-list #check-all').on('click', function(){
	...
	$('.fn-wrap .edit-item').on('click', function(){
		try {
	...
	$(document).ready(function(){
		get_item_list(null, null, null, 1);
