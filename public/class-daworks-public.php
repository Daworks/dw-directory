...
class Daworks_Public {
	...
	public function get_sub_category($c_no, $ref, $lev){
		...
		(int) $ref_n = (isset($_GET['ref_n']))?get_query_var('ref_n', $_GET['ref_n']):"";
	...
	public function get_breadcumb($c_no) {
		...
		elseif ( $row->lev == 1 ) {
			$upper = $wpdb->get_row("select c_no, ref, ref_n, lev, c_title from $table where c_no = $row->ref");
	...
	public function get_data($c_no, $ref, $lev){
		...
		if ( isset($_GET['pno']) ){
			$page = get_query_var('pno', $_GET['pno']);
		}
		else {
			$page = 1;
		}
		...
		$_GET['c_no'] = isset($_GET['c_no'])?$_GET['c_no']:"";
		$_GET['step'] = isset($_GET['step'])?$_GET['step']:"";
		$_GET['ref_n'] = isset($_GET['ref_n'])?$_GET['ref_n']:"";
		if ($page > 1) {
			$page_tag .= '<li class="prev"><a href="'.get_permalink($post->ID).'?&c_no='.get_query_var('c_no', $_GET['c_no']).'&ref='.get_query_var('ref', $_GET['ref']).'&ref_n='.get_query_var('ref_n', $_GET['ref_n']).'&lev='.get_query_var('lev', $_GET['lev']).'&step='.get_query_var('step', $_GET['step']).'&pno='.($page-1).'">이전</a></li>';
		}
		...
		for($i = 1 ; $i<=ceil($total/$per_page); $i++){
			...
			if ( $i == $page ) {
				...
				$page_tag .= '<li><a href="'.get_permalink($post->ID).'?&c_no='.get_query_var('c_no', $_GET['c_no']).'&ref='.get_query_var('ref', $_GET['ref']).'&ref_n='.get_query_var('ref_n', $_GET['ref_n']).'&lev='.get_query_var('lev', $_GET['lev']).'&step='.get_query_var('step', $_GET['step']).'&pno='.$i.'">'.$i.'</a></li>';
			}
		}

		if ($page < ceil($total/$per_page)) {
			$page_tag .= '<li class="next"><a href="'.get_permalink($post->ID).'?&c_no='.get_query_var('c_no', $_GET['c_no']).'&ref='.get_query_var('ref', $_GET['ref']).'&ref_n='.get_query_var('ref_n', $_GET['ref_n']).'&lev='.get_query_var('lev', $_GET['lev']).'&step='.get_query_var('step', $_GET['step']).'&pno='.($page+1).'">다음</a></li>';
		}
